...
Command to run the test locally:
	...
	make test-e2e-node FOCUS="Feature:ReloadKubeletClientCAFile" SKIP="nothing" TEST_ARGS='--ginkgo.v --kubelet-flags="--fail-swap-on=false"'
*/
var _ = SIGDescribe("ReloadKubeletClientCAFileTest", feature.ReloadKubeletClientCAFile, framework.WithNodeConformance(), func() {
	...
	ginkgo.Context("certificate reload", func() {
		...
		ginkgo.BeforeEach(func(ctx context.Context) {
			...
			cfg.FeatureGates = map[string]bool{
				"ReloadKubeletClientCAFile":          true,
				"ReloadKubeletServerCertificateFile": true,
